...
let outRank = 0;
let rankingData = [];
playerIdToScoreSorted.forEach((score, playerId) => {

  if (entryIdToName.get(playerId) == null){ return; }

  rank += 1;
  if (score != prevScore){ outRank = rank; }

  if(outRank < 10){rankingData.push(`${outRank},${playerId},${entryIdToName.get(playerId)},${score}`)}
